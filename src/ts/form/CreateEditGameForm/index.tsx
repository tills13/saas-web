...
class CreateEditGameForm extends React.Component<Props> {
  ...
  render () {
    ...
    const { boardHasGold, boardHasTeleporters, boardHasWalls, selectedSnakes } = formData
    const { boards, daemons, snakes } = application

    const mSelectedSnakes = selectedSnakes && isArray(selectedSnakes) ? selectedSnakes : []

    const hasLegacySnake = mSelectedSnakes
      .map((snakeId) => snakes!.items.find(({ id }) => id === snakeId))
      .reduce((carry, snake) => carry || !!(snake && !!snake.apiVersion), false)
    ...
    const snakeOptions = snakes!.items.map(snake => ({ label: snake.name, value: snake.id }))
